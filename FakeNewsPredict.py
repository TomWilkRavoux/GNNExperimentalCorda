...
def _(mo):
    mo.md(r"""
    ## Test sur nouveau dataset
    #### Ce test a pour but de mettre en condition de prediction un model qui à été entrainer sur des data politiques, pour voir comment il se comporte sur des data de people et en tirer des conclusion de la problement qui est :
    **Est ce que les patterns de diffusion sont universel ou unique spécifique au domaine ?**
    """)
    return
...
def _():
    ...
    base = './data/UPFD/gossipcop'
    raw  = os.path.join(base, 'raw')
    os.makedirs(raw, exist_ok=True)

    needed = {'A.txt', 'graph_labels.npy', 'node_graph_id.npy',
              'train_idx.npy', 'val_idx.npy', 'test_idx.npy'}

    # trouve les fichiers bruts où qu'ils soient sous gossipcop/ et les remonte dans raw/
    for _f in glob.glob(f'{base}/**/*', recursive=True):
        _name = os.path.basename(_f)
        if os.path.isfile(_f) and (_name in needed or _name.endswith('.npz')):
            _dest = os.path.join(raw, _name)
            if os.path.abspath(_f) != os.path.abspath(_dest):
                shutil.move(_f, _dest)

    print("Fichiers dans raw/ :")
...
@app.cell
def _(ToUndirected, UPFD, ckpt):
    gossip_test = UPFD('./data/UPFD', 'gossipcop', ckpt['feature'], 'test',
                       transform=ToUndirected(), force_reload=True)
    print(f"GossipCop test : {len(gossip_test)} graphes")
    print(f"Features/nœud  : {gossip_test.num_features} "
          f"(doit valoir {ckpt['config']['in_channels']})")
    return (gossip_test,)


@app.cell
def _(gossip_test, predit_detail):
    df_gossip = predit_detail(gossip_test)

    print(f"Accuracy transfert (politifact → gossipcop) : {df_gossip['correct'].mean():.4f}")
    print(f"Erreurs : {(~df_gossip['correct']).sum()} / {len(df_gossip)}")
    df_gossip
    return
